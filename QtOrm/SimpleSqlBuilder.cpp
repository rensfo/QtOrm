...
QSqlQuery SimpleSqlBuilder::insertObject(const QObject &object) {
  ...
  QString fullSqlText = QString("insert into %1(%2) values(%3)").arg(classBase->getTable()).arg(columns).arg(values);
...
QSqlQuery SimpleSqlBuilder::updateObject(const QObject &object) {
  ...
  QString fullSqlText =
      QString("update %1 set %2 where %3").arg(classBase->getClassName()).arg(setClause).arg(whereClause);
  QSqlQuery *query = new QSqlQuery(database);
  query->prepare(fullSqlText);
  foreach (auto prop, classBase->getProperties()) {

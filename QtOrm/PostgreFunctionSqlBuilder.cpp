...
QString PostgreFunctionSqlBuilder::getUpdateColumnText(Mapping::ClassMapBase &classBase){
    ...
    for(auto property : classBase.getProperties()){
        ...
        QString columnUpdateText = getExecuteFunctionText(*property, classBase.getIdProperty().getColumn(), classBase.getContext());
...
QString PostgreFunctionSqlBuilder::getAnonymousBlock(QString declare, QString body)
{
    return QString("do$$\n"
                   ...
                   "begin\n"
                   "%2"
                   "end$$;").arg(declare).arg(body);
}

QString PostgreFunctionSqlBuilder::getExecuteFunctionText(const Mapping::PropertyMap &property, const QString &idParamName, const QString &context){
